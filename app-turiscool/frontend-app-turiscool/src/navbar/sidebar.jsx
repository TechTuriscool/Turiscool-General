...
const SideBar = () => {
    ...
    const buttonsTop = [
        ...
        { text: 'Customer Success', alt: 'CustomerSuccess', path: '/customer-success/' },
        { text: 'Guía de uso', alt: 'Guia', path: '/guide' }
    ];
    ...
    const openDropDown = (event) => {
        let alt = event.currentTarget.querySelector('img').alt;
        setSelectedAlt(alt);
        if (alt === 'Guia') {
            navigate('/guide');
        } else {
        ...
        <div className='sideBarContainerButtons'>
            <div className='containerButtonsTop'>
                {buttonsTop.map((button, index) => (
                    <div
                        key={index}
                        className={`buttonSideBar ${location.pathname.includes(button.path) ? 'active' : ''}`}
                        onClick={openDropDown}
                        style={{ background: location.pathname.includes(button.path) ? '#006791' : '#fff' }}
                    >
                        <img src={`data:image/svg+xml;utf8,${encodeURIComponent(location.pathname.includes(button.path) ? menuWhite : menuBlack)}`} alt={button.alt} />
                        <p style={{ color: location.pathname.includes(button.path) ? '#fff' : '333' }}>{button.text}</p>
                    </div>
                ))}
            </div>
            {dropDownVisible && (
                <div ref={dropDownRef}>
                    <DropDown alt={selectedAlt} />
                </div>
            )}
